...
class Phoenix(object):
    ...
    def shutdown(self):
        ...
        try:
            ...
            if isinstance(e, PhoenixError) and '[Errno 10054]' in str(e):
                pass  # Handle specific error
            else:
    ...
    def _send_command(self, command, payload=''):
        ...
        if not self.connected:
            if self.reconnect:
                ...
                print("Not connected and unable to reconnect.")  # Handle the disconnection case
                return  # Exit early if we can't connect

        try:
            sent_at = int(time.time())
            # Ensure the command and payload are properly encoded
            self.sock.send((command + '\x01' + payload).encode())

            while int(time.time()) <= sent_at + int(self.send_timeout):
                res = self.sock.recv(64)
                if res == b'@AHello Housekeeping, Love from Phoenix Emu\x01':
            ...
            if self.reconnect:
                ...
                print("Connection lost, and reconnect is not enabled.")  # Handle the lost connection

        except socket.error as e:
            print(f"Socket error occurred: {e}")  # Log socket errors
            self.connected = False
            if self.reconnect:
                self._try_connect()
                self._send_command(command, payload)  # Retry sending command
            else:
                print("No connection established. Unable to send command.")  # Handle disconnection
